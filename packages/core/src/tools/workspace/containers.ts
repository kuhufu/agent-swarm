...
const ExecuteFileParams = Type.Object({
  ...
  ports: Type.Optional(Type.Array(Type.Object({
    ...
    hostPort: Type.Optional(Type.Number({
      description: "宿主机 localhost 上用于访问服务的端口（同时绑定 IPv4 127.0.0.1 和 IPv6 ::1）。默认等于 containerPort。想把容器内 3000 映射为本机 5174 时，保持 containerPort=3000，只设置 hostPort=5174；不要改应用代码监听端口。",
      minimum: 1,
      maximum: 65535,
    })),
  }), {
    description: "可选端口映射，仅用于启动服务容器。未设置时容器使用 --network none；设置后会启用 Docker bridge 网络，同时绑定到宿主机 IPv4 127.0.0.1 和 IPv6 ::1。containerPort 是应用在容器内实际监听的端口，hostPort 是本机访问端口。只想换浏览器/用户访问端口时只改 hostPort，不要改代码。示例: app 监听 3000，但希望用户访问 http://localhost:5174，则传 [{\"containerPort\":3000,\"hostPort\":5174}]。不要再启动另一个容器用 localhost 访问这个端口。",
    maxItems: 8,
  })),
...
export function createRunWorkspaceContainerTool(
  workspace: WorkspaceManager,
...
function buildPortArgs(ports: PortMapping[]): string[] {
  return ports.flatMap((port) => [
    ...
    "-p",
    `[::1]:${port.hostPort}:${port.containerPort}`,
  ]);
}
...
function formatPorts(ports: PortMapping[]): string {
  return ports
    .map((port) => `localhost:${port.hostPort}->${port.containerPort}`)
    .join(", ");
}
