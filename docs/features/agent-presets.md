...
- `preset_agents`：用户级预设，按 `userId` 隔离，业务主键为 `(user_id, id)`。
...
- `GET /`：列出系统模板
- `POST /`：创建系统模板（需要 `admin`）
- `PUT /:id`：更新系统模板（`id` 不可变，需要 `admin`）
- `DELETE /:id`：删除系统模板（需要 `admin`）
...
- 用户级预设 ID 只要求在当前用户内唯一，不同用户可以复用相同 ID。
- 系统模板是跨用户共享资源，写操作需要管理员权限。
