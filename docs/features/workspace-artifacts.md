...
侧边栏提供独立“工作区”页面，用户可以创建、编辑、归档、删除工作区，并直接查看某个工作区的产物。聊天页顶部提供工作区选择入口，用户可以手动选择已有工作区、创建新工作区或清除当前会话挂载；系统不会为会话自动创建工作区。聊天页右侧提供 Agent、Trace、产物三个 tab。产物 tab 会按目录分组列出当前挂载 workspace 文件，展示路径、类型、大小和更新时间，并支持：
...
- 单文件菜单和批量操作菜单以浮层形式显示，层级高于产物列表卡片。
- 整个工作区打包下载。
